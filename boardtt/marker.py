...
class PlanarCardMarker(CardMarker):
    ...
    def _get_card_coords(self, row_num: int, col_num: int) -> tuple[int, int, int, int]:
        ...
        x = self.config.offset_from_left_border_px + (row_num * (self.config.card_width_px + self.config.offset_x_px))
        x1 = x + self.config.card_width_px

        y = self.config.offset_from_top_border_px + (col_num * (self.config.card_height_px + self.config.offset_y_px))
